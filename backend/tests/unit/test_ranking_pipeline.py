...
def test_competition_submission_pipeline_limits_to_top_100():
    tmp_dir = tempfile.mkdtemp(dir=".", prefix="pytest_tmp_")
    try:
...
def test_competition_submission_pipeline_rejects_invalid_top_n():
    tmp_dir = tempfile.mkdtemp(dir=".", prefix="pytest_tmp_")
    try:
        pipeline = CompetitionSubmissionPipeline(export_dir=Path(tmp_dir))
